...
def format_codepipeline_event(message: Dict[str, Any]) -> Dict[str, Any]:
    """
    Format CodePipeline state change event into Slack message format

    :params message: SNS message body containing event
    :returns: formatted Slack message payload
    """
    ...
    return {
        "color": color,
        "fallback": f"{pipeline} - {stage}::{state}",
        "fields": [
            {
                "title": "Pipeline",
                "value": f"`{pipeline}`",
            ...
            {
                "title": "account",
                "value": f"`{message['account']}`",
                "short": True,
            },
            {
                "title": "time",
                "value": f"`{message['time']}`",
                "short": True,
            },
            {
                "title": "execution-url",
                "value": f"`{detail['execution-result']['external-execution-url']}`",
                "short": True,
            }
        ],
        "text": f"{pipeline} {stage} {state}",
    }
...
def get_slack_message_payload(
    ...
    if "AlarmName" in message:
        ...
        isinstance(message, Dict) and message.get("detail-type") == "CodePipeline Action Execution State Change"
    ):
        notification = format_codepipeline_event(
            message=message
        )
        attachment = notification
