...
resource "aws_cloudwatch_event_rule" "release_rule" {
  count       = var.enable_status_notifications ? 1 : 0
  name        = "${var.project_name}-${var.github_branch_release}-cicd-pipeline"
  description = "${var.project_name}-${var.github_branch_release} CodePipeline execution status"
  ...
  event_pattern = <<PATTERN
{
    ...
    "source": ["aws.codepipeline"],
    "detail": {
        "pipeline": ["${aws_codepipeline.pipeline_release.name}"],
        "state":["SUCCEEDED","FAILED"],
        "type": {
          "category": ["Source","Deploy","Build","Test"]
        }
...
resource "aws_cloudwatch_event_target" "relase_target" {
  count     = var.enable_status_notifications ? 1 : 0
  rule      = aws_cloudwatch_event_rule.release_rule[0].name
  target_id = "SendToSNS"
  arn       = data.aws_ssm_parameter.sns_topic_arn.value
}
